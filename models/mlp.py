...
class MLP0:
    ...
    def forward(self, A0):
        ...
        """
        Z0 = self.layers[0].forward(A0)  # Pass input A0 through the linear layer
        A1 = self.layers[1].forward(Z0)  # Pass Z0 through the ReLU activation layer

        if self.debug:
            self.Z0 = Z0  # Store Z0 for debugging
            self.A1 = A1  # Store A1 for debugging

        return A1  # This should be the final output after activation

    def backward(self, dLdA1):
        """
        Refer to the pseudo code outlined in the writeup to implement backpropagation through the model.
        """
        dLdZ0 = self.layers[1].backward(dLdA1)  # Backward pass through ReLU
        dLdA0 = self.layers[0].backward(dLdZ0)  # Backward pass through the linear layer

        if self.debug:
            self.dLdZ0 = dLdZ0  # Store dLdZ0 for debugging
            self.dLdA0 = dLdA0  # Store dLdA0 for debugging

        return dLdA0  # This should be the gradient with respect to the input A0
...
class MLP1:
    ...
    def forward(self, A0):
        ...
        # Pass through first linear layer and ReLU
        Z0 = self.layers[0].forward(A0)
        A1 = self.layers[1].forward(Z0)
        # Pass through second linear layer and ReLU
    ...
    def backward(self, dLdA2):
        ...
        # Backprop through second ReLU and linear layer
        dLdZ1 = self.layers[3].backward(dLdA2)
        dLdA1 = self.layers[2].backward(dLdZ1)
        # Backprop through first ReLU and linear layer
...
class MLP4:
    def __init__(self, debug=False):
        ...
        self.layers = [
            ...
            Linear(8, 8), ReLU(),
            Linear(8, 4), ReLU(),
            Linear(4, 2)  # Output layer without activation here
        ]
